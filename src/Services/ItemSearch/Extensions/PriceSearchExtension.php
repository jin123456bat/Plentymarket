...
use Plentymarket\Helper\VariationPriceList;
...
class PriceSearchExtension implements ItemSearchExtension
{
	...
	public function transformResult ($baseResult, $extensionResult)
	{
		if (count($baseResult['documents'])) {
			...
			$contactClass = $accountService->getContactClassData($accountService->getContactClassId());
			$customerClassMinimumOrderQuantity = 0;
			if (is_array($contactClass) && count($contactClass) && isset($contactClass['minItemQuantity'])) {
				$customerClassMinimumOrderQuantity = (int)$contactClass['minItemQuantity'];
			}
			...
			foreach ($baseResult['documents'] as $key => $variation) {
				if ((int)$variation['data']['variation']['id'] > 0) {
					...
					if ($variation['data']['prices']['default']['unitPrice']['value'] <= 0 || $variation['data']['prices']['default']['price']['value'] <= 0) {
						$this->getLogger(__CLASS__)->warning('Plentymarket::Debug.PriceSearchExtension_freeItemFound', [
							'variation' => $variation,
